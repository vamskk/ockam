...
ockam_error_t test_responder_handshake(ockam_key_t* p_key, ockam_vault_t* vault,
                                       ockam_reader_t* p_reader, ockam_writer_t* p_writer)
{
  ...
  /* Msg 1 receive */
  error = ockam_read(p_reader, &read_buffer[0], MAX_XX_TRANSMIT_SIZE, &bytes_received);
  if(error) goto exit;
  ...
  if (0 != memcmp(write_buffer, compare, compare_bytes)) { error = KEYAGREEMENT_ERROR_TEST; goto exit; }
  ...
  /* Msg 3 receive */
  error = ockam_read(p_reader, read_buffer, MAX_XX_TRANSMIT_SIZE, &bytes_received);
  if(error) goto exit;
  ...
  /* Epilogue */
  error = xx_responder_epilogue(&xx, p_key);
...
//  key_establishment_xx xx;
  ...
  if (scripted_xx) {
    error = test_responder_handshake(&key, vault, p_reader, p_writer);
  } else {
    error = ockam_key_establish_responder_xx(&key, vault, p_reader, p_writer);
  }
  if (error) {
  ...
  if (scripted_xx) {
    ...
    error = xx_encrypt(&key, test, test_size, write_buffer, sizeof(write_buffer), &transmit_size);
    ...
    error = xx_encrypt(&key, (uint8_t*) ACK, ACK_SIZE, write_buffer, sizeof(write_buffer), &transmit_size);
    if (error != TRANSPORT_ERROR_NONE) {
      log_error(error, "responder_epilogue_make failed");
      goto exit;
    }
  ...
  error = xx_decrypt(&key, test, TEST_MSG_BYTE_SIZE, read_buffer, transmit_size, &test_size);
  if (TRANSPORT_ERROR_NONE != error) {
